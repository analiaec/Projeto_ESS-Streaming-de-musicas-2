...
describe('MusicasService', () => {
  ...
  describe('emAlta', () => {
    ...
    it('deve retornar as 10 músicas mais reproduzidas ordenadas por reproduções', async () => {
        ...
        const musicasMock = [
        { id: 1, titulo: 'Oceano',  genero: 'MPB', reproducoes: 1000 },
        { id: 2, titulo: 'Sina',    genero: 'MPB', reproducoes: 999  },
        { id: 3, titulo: 'Garota',  genero: 'Bossa Nova', reproducoes: 950 },
        ];
    ...
    it('deve retornar no máximo 10 músicas', async () => {
        const musicasMock = Array.from({ length: 12 }, (_, i) => ({
        id: i + 1,
        titulo: `Musica ${i + 1}`,
        genero: 'MPB',
        reproducoes: 1000 - i,
        ...
        mockRepository.find.mockResolvedValue(musicasMock.slice(0, 10));
  ...
  describe('buscar', () => {
    ...
    it('deve chamar andWhere com os parâmetros corretos para gênero', async () => {
    ...
    expect(qbMock.andWhere).toHaveBeenCalledWith(
        'LOWER(musica.genero) = LOWER(:genero)',
        { genero: 'MPB' }
    );
    });
    ...
    it('deve chamar andWhere com ano convertido para inteiro', async () => {
    const qbMock = criarQBMock([]);
    mockRepository.createQueryBuilder.mockReturnValue(qbMock);
  ...
  describe('registrarReproducoes', () => {
    ...
    it('deve incrementar as reproduções e criar um playback', async () => {
        const musicaMock = { id: 1, titulo: 'Sina', genero: 'MPB', reproducoes: 999 };
    ...
    it('deve chamar findOneByOrFail com o id correto', async () => {
        const musicaMock = { id: 1, titulo: 'Oceano', genero: 'MPB', reproducoes: 1000 };

...
export class MusicasService {
  ...
  async buscar(termo?: string, artista?: string, genero?: string, ano?: string): Promise<Musica[]> {
    ...
    if (termo) {
      ...
      query.addSelect(`CASE WHEN LOWER(musica.titulo) = LOWER(:termoExato) THEN 1 ELSE 2 END`, 'relevancia')
          .setParameter('termoExato', termo);
    ...
    if (genero) {
      // generos é um array — verifica se contém o gênero buscado
      query.andWhere(':genero = ANY(album.generos)', { genero });
    }
    ...
    if (ano) {
      query.andWhere('EXTRACT(YEAR FROM CAST(album.data AS DATE)) = :ano', { ano: parseInt(ano) });
    }

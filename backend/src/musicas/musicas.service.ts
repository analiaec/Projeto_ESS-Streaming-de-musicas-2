...
export class MusicasService {
  ...
  async emAlta(): Promise<Musica[]> {
    return this.musicaRepository.find({order: {reproducoes : 'DESC', titulo: 'ASC'}, take:10, relations: ['artistas', 'albuns']});
  }
  async buscar(termo?: string, artista?: string, genero?: string, ano?: string): Promise <Musica[]> {
    const query = this.musicaRepository.createQueryBuilder('musica').leftJoinAndSelect('musica.artistas', 'artista').leftJoinAndSelect('musica.albuns', 'album');
    if(termo) {
    ...
    if(artista) {
        query.andWhere('LOWER(artista.nome) LIKE LOWER(:artista)', {artista : '%' + artista + '%'});
    }
    if(ano) {
